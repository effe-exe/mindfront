...
- Impassable peaks (terrain magnitude 31, shown as land) can never be taken by anyone; a cluster walled by them is as sealed as by water. Unclaimed: flat 16 / 20 / 24 per tile (×fallout); time `clamp(2,000 × C / A, 5, 100) / (2 × front)` → 0.4 × front tiles/tick once A ≥ 6,600 / 8,000 / 10,000; more troops only last longer (≈ A/16 plains tiles).
...
- Warship: 1,000 HP; patrols a 100-tile circle around its patrol point (its Port at birth; `move_warship(id, x, y)` moves the point anywhere on the same water body) and hunts within 130 tiles (`warshipTargettingRange`): transports first (no reload), then warships, then trade ships; shells 200–300 per 20 ticks; never shells land, troops or structures. A captured trade ship pays its whole payout to the captor. It dies only to shells; it heals near one of my Ports and survives the loss of its Port.

## 5. Structures

n = already built of that type, upgrades included. Gold charged at start, never refunded. ≥15 tiles between structures (`structureMinDist`). `upgrade(unit, id?)`: +1 level on a finished structure of mine, instant, same price as the next new one (`UpgradeStructureExecution`, `canUpgradeUnit`), no spacing; City, Port, Factory, Missile Silo, SAM Launcher only (`upgradable`); `id` from `me.units`, omitted = my lowest-level finished one. Level = effect multiplier: City +250k cap per level, Port one trade roll per level, Factory one train per level, Silo/SAM one missile per level. A structure only counts once finished (`me.underConstruction`). A structure changes hands with its tile (Defense Post destroyed instead); a tile made unowned by a nuke destroys it; tribes delete any structure they hold within ~30 s. Structures cannot be moved, sold or deleted; the only exits are capture and nukes.
...
| Defense Post | `min(250k, 50k × (n+1))` | 50 | §3 multipliers on my tiles within 30 (`defensePostRange`) against land attacks and boat landings alike; never shoots at anything (ship targeting is disabled in the engine); nothing against nukes | border tile |
...
- `relation`: my ledger of their acts toward me, −100..100: attack −60, alliance break −100 (−40 with everyone else bordering the breaker), nuke −100, alliance +100; decays 0.05/tick to 0; labels hostile < −50, distrustful < 0, neutral < 50, else friendly. No mechanical effect on AI seats; tribes ignore it; nations act on THEIR ledger of me, shown as `relationToMe` (hostile → they embargo me and I am their preferred attack and nuke target; friendly → they accept my alliance).
...
- `break_alliance`: traitor 300 ticks (`traitorDuration`, countdown in `traitorTicksLeft`), `betrayals` +1 forever; no mark if the other side is already a traitor (breaking with a traitor is free). Traitor: §3 multipliers against me; each bordering tribe attacks me at 1-in-3 odds per attack tick (1-in-6 if allied, breaking it). Cascade: on Medium and up every allied nation whose troops are ≥ 83% of mine breaks with me at its next act tick (≤6 s on Hard) and attacks; `me.betrayalCascade` lists them now. Terra lost a won solo match this way: one break at 18:00 with 28k tiles, two allies turned in 2 s, 5k tiles at the buzzer.
- Tribes (`kind: "tribe"`, 120 bots): act every 40–80 ticks; accept every alliance and extension request; never build (they delete structures they capture); hunt a bordering traitor with 1/3 odds at any troop level (1/6 and breaking their own alliance if allied); expand while free land borders them; otherwise act only at ≥50–60% of cap, sending everything above a 30–40% reserve: first at their largest attacker, else a bordering traitor (1/3), else a random non-allied neighbour, skipping AI seats and nations half the time; can boat; difficulty changes nothing for them. Weak on paper: cap ÷3, regen ×0.5, attacker losses ×0.7 against them. Conquered → all their gold (50/tick ≈ 30k per minute alive).
- Nations (`kind: "nation"`, only when the match enables them; real countries spawning near their true position during the spawn phase, hopping around it until it ends): scripted like tribes (`AiAttackBehavior`: attack at ≥50–60% of cap, keep a 30–40% reserve, boats too) but full players: gold 100/tick like AI seats, same spawn immunity, keep and build structures (Cities first; Ports/Factories ≈0.75 per City; Silos 0.2 per City, max 3; SAMs 0.15–0.3 per City), send emojis, embargo whoever their ledger rates hostile (`relationToMe`), nuke with a silo and the gold (Hydrogen if affordable else Atom; at an ally's target, else the most hated hostile player unless its cap is under half theirs, else the tile leader once ahead of them by the threshold below; never at tribes), conquered → all their gold. `game_info.difficulty` sets the rest:
  - Easy: start 12,500 troops, cap ×0.5, regen ×0.9, act every 65–100 ticks; no Defense Posts; a Warship at 1/50 odds per act tick with a Port; attack order: whoever nuked them, tribes (one at a time), retaliation, an ally's target, betrayal, most hated, weakest; an attack on an AI seat goes ahead only 1 time in 4. Alliances: 10% random; traitor refused 90%; accept when their ledger of me is friendly, or in the first 5 min (90%), or when I hold ≥60–70% of their troops or ≥70–80% of their tiles; offers to bordering players at 1/30 odds; never betray an AI seat; embargo lifts when their ledger reaches neutral; nuke the leader at 40 points ahead.
  - Hard: start 25,000, cap ×1, regen ×1 (an AI seat's economy), act every 45–60 ticks; Defense Posts on active fronts (up to ceil(ratio/0.4)); Warships with a Port and they track incoming ships and retaliate; attack order: tribes (3 at once), retaliation, an ally's target, betrayal, whoever nuked them, a traitor with < 1.2× their troops, AFK, most hated (< 3× their troops), an enemy under 15% of its cap, the juiciest (≤75% of their troops, most structures), a victim already under attack for > 50% of its troops, weakest, islands, donations to allies; no attack on a player unless it sends ≥20% of that player's troops (retaliation exempt); attacks on AI seats never skipped. Alliances: 2.5% random; traitor refused 90%; refuse anyone allied with ≥50% of the non-tribe players; ACCEPT a threat (more troops than them and > 2× their cap); refuse below neutral; friendly → 83%; refuse when they already have 3–4 alliances or every other non-tribe neighbour is already a friend; first 3 min → 50%; else when I hold ≥75–85% of their troops or ≥85–95% of their tiles. Betray an ally whose troops (incl. attacks) fall under 20% of its cap while weaker than them, a traitor ally under 1.2× their troops, or their only bordering player at 3× weaker. Embargo lifts only when their ledger reaches friendly. Nukes avoid trajectories a SAM can intercept and hit the leader at 20 points ahead; MIRV when 25M is on hand.
- `donate(target, troops|gold)`: allies only; one donation per ally per 100 ticks, gold and troops share the cooldown (`donateCooldown`); troops capped at the room under their cap and at my stock; their relation to me +50 for ≥ ~1/12 of their cap in troops, +5 per 2,500 gold (chunk grows ~×2 per 5 min), max +100 (`DonateTroopsExecution`, `DonateGoldExecution`). `embargo(target, stop?)`: no trade ship between my Ports and theirs (both lose that income) until I lift it; a nation counts it as −20 relation. The 5-min embargo after an attack (§3) is the engine's and lifts itself. Target calls: no tool.
...
| `embargo` | `target`, `stop?` | visible id | trade stops both ways (§7) | not visible; already embargoed / none to lift |
| `move_warship` | `id`, `x`, `y` | a Warship id in `me.units`; water on its sea | patrol point moves (§4) | not your Warship; land; other water body; off map |
...
| `me.allies`, `me.betrayalCascade`, `me.allianceExpiry[{id,ticksLeft,theyAgreedToExtend,iAgreedToExtend}]`, `me.pendingAllianceRequestsFrom`, `me.pendingRequestExpiry[{id,ticksLeft}]` | allies; allied nations that turn on me the moment I break any alliance (§7); ticks until each expires and who has asked to renew; offers awaiting me and ticks until they lapse | which border is frozen, how long; `betrayalCascade` non-empty → do not break anything; `extend_alliance` when `ticksLeft` ≤ 300 or they asked; accept / reject |
...
6. IF `reachableByBoat` has a tribe with high `gold` and low `troops/tiles`: `boat` with troops above their army, then `expand` from the beachhead. Nations (§7): on Easy a slower AI with half the cap that allies 90% in the first 5 min and never betrays me; on Hard a full economy that retaliates every time, refuses the over-allied, and betrays an ally that runs its troops down: never sit under 20% of cap next to one, keep my alliance count under half the field, and take it late for all its gold.
...
| When | Then |
| --- | --- |
| Allied | no attacks either way (one in flight retreats, no malus); a boat landing on them turns back; nukes in flight between us are deleted, a blast on ≥100 weighted tiles or any structure of theirs breaks it (traitor); temporary embargoes lifted, trade and trains at the ally rate; `donate`, `extend_alliance` open; no shared vision, income or cap |
| Traitor (300 ticks) | attackers lose ×0.5 and move ×0.8 against me; bordering tribes hunt me 1/3 per act tick; nations refuse my offers 90% and may break with me; `betrayals` permanent |
| I attack X | X's ledger of me −60, X embargoes me 5 min, X's pending offer to me is rejected; tribes/nations retaliate first against their largest attacker |
| Under 100 tiles | dead at the next tile lost, all land redistributed, gold to the conqueror (all from a tribe/nation, half from an AI seat, none from one that never attacked) |
| Nuke lands | every owner's tiles in the outer radius stripped (mine too if I aimed badly: the arena refuses that), every structure inside deleted (a City → cap −250k → regen negative above cap), fallout stays: ×(5 − 2 × share) attack cost, out of the win denominator; victim's ledger of me −100 |
| Spawn immunity (50 ticks after the spawn phase) | AI seats cannot attack seats or nations; nobody can nuke; tribes and nations attack anyone; boats can sail but a landing on an immune seat is refused |
| Embargo | no trade ships either way (both incomes drop); nations: −20 relation; lifted by alliance (temporary ones) or `embargo(stop)` |
| Port lost (captured/nuked) | its trade stops, Warships already at sea keep patrolling; rail station gone |
| City lost | cap falls at once; troops above cap bleed off |
| Defense Post | ×5 losses / ×3 time for attackers on my tiles within 30, land and beach alike; nothing vs nukes, ships, or when captured (it is destroyed) |
| SAM | intercepts warheads landing within 70 tiles (level 1), never the MIRV carrier; 90-tick reload per level: N+1 bombs at once beat a level-N SAM |
