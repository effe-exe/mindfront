...
- Tribes (`kind: "tribe"`, 120 bots): accept every alliance request; never build; expand while free land borders them; else attack only at ≥50–60% of cap: attackers first, else a random non-allied neighbour (AIs skipped half the time); can boat. Conquered → all their gold (≈30k per minute alive).
...
4. IF `gold` covers the bottleneck: City when the cap throttles; Port when coastal and a partner Port or boats matter; Defense Post `at` = the pressing neighbour; Factory once a City/Port stands within 110 tiles; SAM only when a rival's `structures["Missile Silo"]` > 0. Idle gold earns nothing; a second Port costs double.
