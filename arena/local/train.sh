...
#   [MODEL=..] [CFG=lora-q3.yaml] [LAYERS=16] [GC=--grad-checkpoint] [DATA=data] [MAXSEQ=4608] [EVERY=500] arena/local/train.sh [adapter name=v1] [iters=2500]
set -e
cd "$(dirname "$0")"
...
CFG=${CFG:-lora-q3.yaml}; LAYERS=${LAYERS:-16}; GC=${GC---grad-checkpoint}
mkdir -p adapters
.venv/bin/python -m mlx_lm.lora --model "$MODEL" --train --data "$DATA" -c "$CFG" \
  --fine-tune-type lora --num-layers "$LAYERS" --batch-size 1 $GC --mask-prompt \
