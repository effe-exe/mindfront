...
export async function runPlayerWithClient(client: Client, opts: RunPlayerOpts): Promise<void> {
  ...
  const baseSystemPrompt =
    ...
    "\nYou act by calling tools. Call `observe` any time for fresh state, `inspect_player` for details, action tools to act, and `say` once per round with 1-2 punchy in-character sentences for the spectators. Stop calling tools when you are done for this round.";
