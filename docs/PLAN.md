...
- 2026-09-14: solo aFYyvMJX8B (GPT-5.6 Terra vs 12 Hard nations): India won on the timer. At 18:00 India 47.4% (21.6M gold), Sudan 41.2% (12.1M), Terra 11.2% (795k gold, 28k tiles, 8 Cities, 8 Ports, 5 Warships, 3 silos, 2 SAMs, 5 nation alliances); Terra then broke with Taiwan (0.1%) to land-grab and Sudan + India (Hard: traitor ally under 1.2× their troops) betrayed it within 2 s → 5k tiles. Lost economically by minute 12, not on the betrayal. Added `me.betrayalCascade` (allied nations that turn if I become a traitor), the cascade rule in §7 and the tool text, and an endgame default that never breaks alliances. Spend $78.16.
- 2026-09-14: `--budget <usd>` per match (OpenRouter `usage.cost` summed across seats; seats stop calling models at the ceiling; spend printed every 100 ticks and in the summary). `--teams`: engine Humans-vs-Nations team mode (every seat on team Humans, `--nations` defaults to the seat count), `me.team`, `teammate` per view, team guards on attack/nuke/ally, guide §13. `arena/roster.team.json` (Terra, Haiku 4.5, Gemini 3.1 Flash Lite, Grok 4.3, Qwen 3.7 Plus, GLM 5.3 Flash). Guide §11.14–16 from the solo loss: embargo the leader (trade pays both), parallel tribe attacks, first-5-minutes plan vs Hard nations. djmango/openfront-ai checked: an RL agent (PPO + spatial autoencoder), no LLM; its "exact bypass" of low-dimensional facts matches our observation design; nothing to port.
- 2026-09-14: team challenge af23ZiAaAH (6 seats vs 6 Hard nations, Humans-vs-Nations mode): team Nations hit the 80 % line at 3:48 of play (Pakistan 23.9 %, Morocco 18 %, South Africa 13.7 %, Iceland 12.2 %, Uruguay 8.2 %); in team mode nations never fight each other and attack without the 20 % check, so the snowball is ~6× the FFA pace; seats held ~30k tiles combined. Cost $1.84 (budget cap and per-response cost accounting verified live). Spend $80.

### Phase RL: a learning agent (proposed 2026-09-14, not started)
Goal from the user: a trained model that beats any human. Facts: the engine is deterministic, headless, full-information and fast (djmango reports ~2,100 ticks/s per env); action surface = the 18 intents already guarded by `arena/observe.ts`. Prior art: github.com/djmango/openfront-ai (no license file; design public; datasets on HF `djmango/openfront-snapshots`, 375k bot + 420k human frames): spatial autoencoder (32ch @ 1/8) + PPO (6M params, Rust), factored action heads with engine legality masks, strength-index reward; after months its agent peaks at ~13k tiles vs 80 Medium bots and dies. Lesson: RL from scratch is slow; imitation first.
Plan: (A) env = gym wrapper over our `GameRunner` (N Node workers, shared-memory tensors), obs = downsampled owner/terrain/structure/fallout grid + the exact scalars `observe()` already computes, factored heads (type × player × 1/8-res tile pointer snapped to the best legal tile × unit × quantity) masked by our existing guards; (B) behavioural cloning from OpenFront's public archive (deterministic records → every human decision labelled; filter winners/top ranks) on the Mac; (C) self-play PPO league on a cloud GPU (nations Hard + past checkpoints + LLM seats), strength-index reward, curriculum small maps → World; (D) ladder: win rate vs Hard nations, vs the LLM roster (the agent is an "external" MCP seat), then invited humans in private lobbies (public lobbies: ToS/anti-bot, do not). Budget: BC on the Mac ≈ $0; PPO ≈ $300–1,500 of GPU time per serious run; timeline weeks to first agent that beats Hard nations, months for top-human level, "any human" is a research bet.

### Phase L: local fine-tuned seat (started 2026-09-14, $0)
Decided: the LLM arena stays; no RL. A local Qwen (mlx-community/Qwen3.6-35B-A3B-4bit, MoE) becomes a seat via `MINDFRONT_LLM_URL` (mlx_lm.server: OpenAI tool calls + prefix cache, thinking off) and is behaviour-cloned from archived human games: `arena/local/fetch.ts` (public API, biggest FFA lobbies), `arena/local/dataset.ts` run inside `~/mindfront-replay` (git worktree at the archive's engine commit 577819ba0; `sync-worktree.sh` + `shim-observe.py` adapt `observe.ts` to that engine) → mlx-lm "tools" JSONL: short system prompt (`arena/local/prompt.ts`), compacted observation (~2.7k tokens), human intents mapped to our tools in 50-tick windows, 10 % idle rounds. `arena/local/train.sh` (LoRA, mask-prompt, batch 1, grad checkpoint), `arena/local/serve.sh`, `arena/roster.local.json`. First record: 635 examples from one 35-min game, in sync. Evaluation = one local seat vs nations (Medium, then Hard), untuned+manual vs tuned+short prompt, free.
- 2026-09-15: local model on disk (20.4 GB via curl; the HF xet downloader stalled at 0 bytes), served by `mlx_lm.server` (tool calls + prefix cache; `model: "default_model"`). Dataset: 121 human games, all in sync in the pinned worktree, 58,325 train / 2,367 valid examples (~4.9k tokens each, 283M total; first LoRA run samples 2,500). Baseline aUjFaMR5aP: untuned Qwen + full manual vs 6 Medium nations → dead at 16:11, peak 20k tiles, no alliances (`arena/local/RESULTS.md`). Aider + a thinking-on coding server (`serve-coding.sh`, :9301, `aider.sh`) for the user's coding tests.
- 2026-09-15: LoRA on Qwen3.6 (35B-A3B and 27B alike) is not feasible in mlx-lm 0.31: their hybrid gated-delta-net layers train through a per-token Python loop that OOMs at ~3k tokens (two router gradient bugs patched in the venv on the way). Switched the trainable model to mlx-community/Qwen3-30B-A3B-Instruct-2507-4bit (qwen3_moe, standard attention, 17 GB). Local seat prompt trimmed: 18 action tools only, observation ~1.5k tokens (no build table, far dossiers 13 fields, ≤8 units); dataset rebuilt with it; `refilter.py` drops over-long examples (a truncated label = NaN loss).
- 2026-09-15 18:00: LoRA v1 running on Qwen3-30B-A3B-Instruct-2507-4bit (router VJP patch applied to the venv's qwen3_moe.py as well): attention-only adapters rank 16 on the last 16 layers, batch 1, grad checkpoint, max-seq 4608, 5,000 iterations ≈ 9 h (6.6 s/iter, peak 24.7 GB). Smoke: val loss 1.73 → 1.02 in 10 iterations. Dataset after the length filter: 25,129 train / 1,680 valid, median 3,933 tokens.
- 2026-09-16 05:30: LoRA v1 done (val 1.73 → 0.14). Tuned seat answers in 4 s with no narration and human-style multi-calls, but in the arena it re-sent the same attack and boats at ratio 0.8 every second and was dead at 1:02: the dataset had only 5 % idle rounds. Dataset v2 emits every empty 50-tick window as a "Holding." example (true ~50 % idle rate); rebuild → filter → LoRA v2 chained, ETA ~14:00.
- 2026-09-16 16:45: **every local match so far ran the untuned base model.** `mlx_lm.server` 0.31.3 resolves `default_model` to the model path and then looks the adapter up under that path, so `--adapter-path` was silently dropped (venv patch in `server.py` `ModelProvider.load`, one line). Found by an offline check (`scratchpad/offline_eval.py`: served model vs 60 held-out labels): the "tuned" seat never held, sent 3 calls at ratio 0.5 and used `donate` (4 examples in 61k). With adapters really loaded: greedy decoding collapses (v2-3000 holds 60/60, v1 sends `build` ×298), so local seats now sample at `MINDFRONT_TEMP` (default 1); v1 at T=1 reproduces the human labels (ratio 0.22, same tool mix, 1.7 calls/round); v2-3000 at T=1 acts only 17 % of rounds (learned the 60 % "Holding." prior more than the conditional; val loss is dominated by format tokens, so 0.10 said nothing). Also fixed in `player.ts`: one assistant turn per observation for local seats (the round loop used to re-call the model with tool results, a distribution it never saw, and it re-sent the same attacks), `lastResult: ""` and no persona line (as trained), `MINDFRONT_DUMP=<file>` to capture requests. Match a3unmCxPoB (v1 adapter, T=1, 6 Medium nations): peak 11,467 tiles at 5:50, dead at 9:11 — 66 attacks at 0.05–0.4, 16 boats, 2 retreats, a Port, a Defense Post, 4 alliance requests; lost a two-front war against nations 6 and 7. Untuned+manual baseline still ahead (20k, 16 min) — it turtles.
- 2026-09-16 16:20: **v3 pipeline** (user: "go, build v3 and train it after v2"). Dataset v3: 300 more archive games fetched (26.8k candidates in 6 days; the 121 we had were all in sync), winner + top-2 survivors (`--keep 2`), `myRecentActions` (the human's own calls of the last 10 s, `{ago, name, args}`) in every observation — at inference `player.ts` keeps the seat's calls the same way, the first idle window after activity always kept and long pauses sampled at 30 % (`--idle 0.3`; 2-game test: 36 % "Holding.", 69 % of examples with recent actions). Observation trimmed further in `compactObs` (≤8 neighbours without derived fields, ≤3 far dossiers of 10 fields, ≤6 units as id/type/x/y, no `game`/`buildCosts`/`nukes`/events/plan/lastResult): 1,786 tokens median (was 2,315) — the tools block (~1.2k) is the floor. `arena/local/offline_eval.py` kept in the repo (the check that found the unloaded adapter). Chain: v2 finishes → v2 offline check at T=1 → LoRA v3 8,000 iterations on `data-v3` (max-seq 3584, checkpoints every 1000) → offline check → one match vs 6 Medium nations.
- 2026-09-16 20:25: LoRA v2 done (val 0.149 at 5000; step-3000 0.100). Offline check of the final adapter at T=1: acts in 10 % of held-out rounds (step 3000: 17 %) — v2 learned "Holding." (60 % of its data), not the decision; shelved. Dataset v3 built from 421 games (398 in sync, 23 on a newer engine build diverge at tick 211 and drop): **198,957 train / 10,513 valid**, 35 % holds, 70 % with recent actions, ratio median 0.20; build 115k, attack 67k, upgrade 29k, boat 22k, extend 19k, expand 19k, ally 13k, nuke 8k. Full examples are 3,470 tokens median / 4,561 max once the chat template renders the tools (my first filter at 11k chars had cut the dataset at its median, biased to early game — caught after 25 iterations, refiltered at 14.5k chars = everything, max-seq 4608). LoRA v3 started 20:22: 8,000 iterations, checkpoints every 1,000, same attention-only rank-16 adapters. Chain: offline check at T=1, then a match vs 6 Medium nations.
- 2026-09-17: LoRA v3 done (val 0.151 at 8000; offline at T=1: acts when the human acted 26/30, holds 29/30 — v1 28/4, v2 3/27; calls/round 0.78 vs 1.2; ratio 0.19 vs 0.22). Two matches (aeT2RhGvbc, aEdZ86Y2a6): dead at 7:56 and 5:53, peaks 4,940 / 7,547. Fixed on the way: the local seat now skips the spawn phase (its own pick was an 18-tile corner; the brain places it far from everyone with room to grow) and only executed calls enter `myRecentActions` (a refused call in memory became a loop). What remains is (1) grounding: half its calls reference things the observation rules out (unaffordable builds, non-neighbour attacks, ids it does not own) — 8k steps is 4 % of a pass over the data; (2) the demonstrators: winner + top-2 survivors of public lobbies sit on a full army and skip the City 94 % of them build first. Decided (user: "do everything"): `offline_eval.py` gains a grounded-calls metric and top-p; v3 decoding variants and checkpoints 3000/5000 compared offline; dataset v4 = winners only (all 421 records have a player winner); LoRA v4 on **Qwen3-4B-Instruct-2507 bf16** (dense, same chat template as the 30B-A3B-2507) with adapters on attention+MLP of all 36 layers (`lora-4b.yaml`) — the point is 5–10× more steps per night than the MoE allows. `mlx_lm.server` venv patch #2: a tool call without `arguments` no longer crashes the request.
- 2026-09-18: state at stop (user: "stop everything we good"). Offline decoding check on v3: T=1/top-p 0.9 keeps act/hold at 55/60 and lifts grounded calls to 92 % (label 88 %); now the local default (`MINDFRONT_TOP_P`). Checkpoints: v3-3000 24/60, v3-5000 35/60, v3-8000 55/60 — the decision was learned late, val loss said nothing. Qwen3-4B bf16 dense: 13 s/step (slower than the MoE's 7 s: full backward, all 36 layers), OOM without checkpointing — dropped. v4a = v3 + 2k steps at lr 5e-5 (warmup, cosine; `lora-q3-hi.yaml`): 56/60, grounded 88 %, val 0.122. Dataset v4 (winners only): 100,626 train / 5,867 valid. v4 (v4a + winners-only) got 50 min of compute overnight — the Mac slept (no caffeinate) — restarted as 4,000 steps, then stopped at the user's request at step ~50. Matches with v3: aeT2RhGvbc 4,940 / 7:56, aEdZ86Y2a6 7,547 / 5:53, aprtLdS9q8 ~4,400 / 4:14 (top-p 0.9: refusals 48 % → 21 %; still attacks a nation at troop parity with a full army instead of eating the tribe next door). Adapters on disk: v1, v2, v3 (+3000/5000), v4a. To resume: `caffeinate -dimsu` first, then `CFG=lora-q3-hi.yaml RESUME=adapters/v4a/adapters.safetensors DATA=data-v4 EVERY=1000 ./train.sh v4 4000`.
